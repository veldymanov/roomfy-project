 ...
 **/
const CACHE = 'templateCache-v0';

/**
 * Set main site/page loading url ( for offline-first )
 * */
...
self.addEventListener('install', event => {
    ...
    event.waitUntil( caches.open(CACHE)
        .then( cache => {
            cache.addAll([
//                'index.html',
//                'external_libraries/jquery/jquery.js'
            ])
        })
    );
});
...
self.addEventListener('fetch', event => {
    ...
    if (url.origin == location.origin && url.pathname == '/'){
        event.respondWith( caches.match(indexHTML)
